...
Поддерживает темы (topics) — отвечает в той теме, откуда пришла команда.
"""
...
manager = GameManager()
buzzer_messages = {}  # chat_id -> message_id
...
async def safe_send(chat_id: int, text: str, bot: Bot, thread_id: int | None = None, reply_markup=None, parse_mode=ParseMode.MARKDOWN):
    """Безопасная отправка с поддержкой тем."""
    try:
...
def _apply_callbacks(game: Game, bot: Bot, thread_id: int | None):
    """Привязывает callback-функции к игре."""
    ...
    async def show_board_callback(g):
        ...
        await safe_send(g.chat_id, board_text, bot, thread_id, reply_markup=keyboard)
    ...
    async def show_scores_callback(g):
        text = g.get_scores_text()
        round_name = g.current_round.name if g.current_round else "Раунд"
        header = "📊 *Конец раунда: {}*\n\n".format(round_name)
        await safe_send(g.chat_id, header + text, bot, thread_id)
...
async def cmd_help(message: Message):
    text = (
        ...
        "• Бот задаёт вопрос\n"
        "• Кто хочет ответить — жмёт кнопку 🔔\n"
        ...
        "📦 *Паки вопросов:*\n"
        "Создавайте на vladimirkhil.com/si/siquester\n"
        "и отправляйте .siq файл в чат ИЛИ кладите в папку packs/\n\n"
        "⏱ *Таймауты:*\n"
        "На кнопку: {} сек\n"
        "На ответ: {} сек"
...
async def cmd_newgame(message: Message):
    ...
    if manager.has_active_game(chat_id):
        await safe_send(chat_id, "⚠️ Уже идёт игра! /stop чтобы остановить.", message.bot, thread_id)
        return
    ...
    await safe_send(chat_id, text, message.bot, thread_id)
...
async def cmd_join(message: Message):
    ...
    if game is None or game.state != GameState.LOBBY:
        await safe_send(chat_id, "⚠️ Нет лобби. Используйте /newgame", message.bot, thread_id)
        return
    ...
    if game.add_player(user.id, username, display_name):
        players_list = '\n'.join(
        ...
        await safe_send(chat_id, "ℹ️ {}, вы уже в игре!".format(display_name), message.bot, thread_id)
...
async def cmd_leave(message: Message):
    ...
    if game.remove_player(user.id):
        await safe_send(chat_id, "👋 *{}* покинул игру.".format(user.first_name), message.bot, thread_id)
    else:
...
async def cmd_startgame(message: Message):
    ...
    if game.get_player_count() < 1:
        await safe_send(chat_id, "⚠️ Нужен хотя бы 1 игрок! /join", message.bot, thread_id)
        return
...
async def cmd_listpacks(message: Message):
    """Показывает список доступных .siq файлов в папке packs/"""
    chat_id = message.chat.id
    thread_id = get_thread_id(message)
    ...
    if not files:
        await safe_send(chat_id, "📁 Папка packs/ пуста.\nПоложите туда .siq файлы и используйте /loadpack <имя>", message.bot, thread_id)
        return

    lines = ["📁 *Доступные паки в packs/:*\n"]
    for i, f in enumerate(files, 1):
        full_path = os.path.join(PACKS_DIR, f)
        size_mb = os.path.getsize(full_path) / (1024 * 1024)
        lines.append("{}. `{}` ({:.1f} МБ)".format(i, f, size_mb))

    lines.append("\n💡 Используйте /loadpack <имя_файла> для загрузки")
    await safe_send(chat_id, "\n".join(lines), message.bot, thread_id, parse_mode=ParseMode.MARKDOWN)


@router.message(Command("loadpack"))
async def cmd_loadpack(message: Message):
    """Загружает .siq файл из локальной папки packs/"""
    chat_id = message.chat.id
    thread_id = get_thread_id(message)
    ...
    if not os.path.exists(file_path):
        await safe_send(chat_id, "❌ Файл `{}` не найден в packs/.".format(file_name), message.bot, thread_id, parse_mode=ParseMode.MARKDOWN)
        return

    if not file_name.lower().endswith('.siq'):
        await safe_send(chat_id, "❌ Файл должен быть в формате .siq", message.bot, thread_id)
        return
...
async def cmd_packinfo(message: Message):
    ...
    pack = manager.get_pack(chat_id)
    if pack is None:
        await safe_send(chat_id, "📦 Пак не загружен. Отправьте .siq файл или используйте /loadpack", message.bot, thread_id)
        return

    info = get_pack_info(pack)
    await safe_send(chat_id, info, message.bot, thread_id, parse_mode=None)
...
async def handle_callback(callback: CallbackQuery):
    ...
    if data == "buzzer":
        ...
        if success:
            ...
            await callback.answer("Кто-то уже отвечает")
        return
